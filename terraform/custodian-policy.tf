...
data "template_file" "cloud_custodian_ebs_copy_instance_tags" {
  template = "${file("template/cloud-custodian/policy/aws-ebs-copy-instance-tags.tpl")}"
  vars = {
    cloud_custodian_role_arn = "${aws_iam_role.cloud_custodian_role.arn}"
  }
}

resource "local_file" "cloud_custodian_ebs_copy_instance_tags" {
  content  = "${data.template_file.cloud_custodian_ebs_copy_instance_tags.rendered}"
  filename = "../custodian-aws-ebs-copy-instance-tags-policy.yml"
}
